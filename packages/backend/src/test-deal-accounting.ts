...
async function testDealAccountingSystem() {
	try {
		...
		// 16. Bilateral deal with manual entries (anti-spam mechanism)
 ...
 */
async function testBilateralDealWithManualEntries(db: any, dealAccountingService: DealAccountingService, accountingService: AccountingService) {
	try {
		...
		await seedService.seedDatabase(COMPANY_A_LEGAL_ENTITY_ID, COMPANY_A_USER_ID);
		console.log("✅ Company A: Chart of accounts seeded");

		await seedService.seedDatabase(COMPANY_B_LEGAL_ENTITY_ID, COMPANY_B_USER_ID);
