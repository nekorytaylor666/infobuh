  ...
  try {
    ...
    if (isLegalEntityScoped && testLegalEntityId) {
      fullPath = path.includes("?")
        ? `${path}&legalEntityId=${testLegalEntityId}`
        : `${path}?legalEntityId=${testLegalEntityId}`;
    }
...
async function testAccountingSystem() {
  ...
  try {
    ...
    const currenciesResponse = await makeRequest<Currency[]>(
      "/currencies",
      {},
      false
    );
    ...
    const hierarchyResponse = await makeRequest<Account[]>(
      "/accounts/hierarchy"
    ); // Will add legalEntityId
    ...
    const entriesResponse = await makeRequest<JournalEntry[]>(
      "/journal-entries"
    );
    console.log(
      "✅ Journal entries:",
    ...
    console.log(
      "  Total Debits:",
      (trialBalanceResponse.data as any)?.totals?.debits || 0
    );
    console.log(
      "  Total Credits:",
      (trialBalanceResponse.data as any)?.totals?.credits || 0
    );
    console.log(
      "  Balanced:",
    ...
    console.log(
      "  Assets:",
      (balanceSheetResponse.data as any)?.assets?.total || 0
    );
    console.log(
      "  Liabilities:",
    ...
    console.log(
      "  Equity:",
      (balanceSheetResponse.data as any)?.equity?.total || 0
    );
    ...
    console.log(
      "  Revenue:",
      (incomeStatementResponse.data as any)?.revenue?.total || 0
    );
    console.log(
      "  Expenses:",
      (incomeStatementResponse.data as any)?.expenses?.total || 0
    );
    console.log(
      "  Net Income:",
      (incomeStatementResponse.data as any)?.netIncome || 0
    );
    ...
    if (createResponse.success && createResponse.data) {
      ...
      if (postResponse.success) {
        ...
        if (ledgerResponse.success && ledgerResponse.data) {
          console.log(
            `✅ Bank account ledger has ${
              (ledgerResponse.data as any[]).length
            } transactions`
          );
          ...
          if (transactions.length > 0) {
            ...
            console.log(
              `   Latest: ${latestTx.transactionDate} - ${latestTx.description} | Debit: ${latestTx.debitAmount} | Balance: ${latestTx.runningBalance}`
            );
          }
        }
        ...
        const trialBalanceResponse = await makeRequest(
        ...
        if (trialBalanceResponse.success && trialBalanceResponse.data) {
          ...
          console.log(
            `   Debits: ${data.totals?.debits || 0}, Credits: ${
...
setTimeout(async () => {
  ...
  const entry = {
    entryNumber: "1749039158915",
    entryDate: "2025-06-04",
    description: "АВР",
    reference: "250240008548",
    currencyId: "ab75a586-5609-437c-abf9-249b304bbefb",
    lines: [
      {
        accountId: "e1929fca-a409-478a-8b4f-2913f3763454",
        debitAmount: 1500000,
        creditAmount: 0,
      },
      {
        accountId: "1fb6bf4f-86b4-4e4d-9f71-35cec767d12a",
        debitAmount: 0,
